...
| D8 | **The application has never been run against a real database** | Narrower than it was. Of 96 tests, `ApplicationBootTests` and `IssueOwnershipEndpointTests` now boot the host and exercise the HTTP pipeline, the authentication scheme and both health endpoints — so those are observed rather than read from source. What is still unobserved is the part that needs a server: every test runs on the in-memory provider, so no migration has been applied to a real schema and no relational query has been executed. F6's `FindAsync` bug is the standing evidence for why that distinction matters: it compiled cleanly for three years and threw the first time it ran. | [#16](https://github.com/konradcinkusz/quorum/issues/16), [#17](https://github.com/konradcinkusz/quorum/issues/17) |
...
| O6 | **Clear or re-upload any signed petition document stored before 2026-09-02** | Open. #19 made delivery authenticated for *new* uploads; it cannot reach assets already stored under the old public type, whose URLs stay live on the CDN regardless of what this repository does. Plausibly an empty set — there is no deployed environment — but that cannot be checked from here, only in the Cloudinary console. |
...
| 16 — a test project covering the logic-bearing layer | ❌ F10 | ✅ 96 tests — logic, plus a booted host on the in-memory provider. What is left is D8. **Read this off the test runner, not off arithmetic**: it has been wrong twice for that reason. |
...
| F6 (open half) — signed documents delivered publicly | ❌ D4 | ✅ 2026-09-02. Stored with Cloudinary's `authenticated` delivery type, and reached through an endpoint applying `SignedDocumentAccess.RestrictToSignatory` — the same predicate upload uses, now called rather than copied. The upload call no longer returns the document's public URL. **Assets uploaded before that date are not covered**, which is O6. |
| F14 — `iTextSharp` 5.x (AGPL, .NET Framework-era, BouncyCastle) | ❌ D7 | ✅ Replaced with PDFsharp + MigraDoc 6.2.4, both MIT, per [ADR 0002](0002-pdf-generation.md) ([#21](https://github.com/konradcinkusz/quorum/issues/21)). `WrtiePDFDocumentToFile` — never called, and invalid if it had been — was deleted rather than ported; generation is now covered by tests that parse the bytes back; and BouncyCastle's departure was confirmed by walking the 541-package transitive closure rather than inferred from a silent build log. |
