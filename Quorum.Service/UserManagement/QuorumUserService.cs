...
/// <para>
/// These used to live on <c>QuorumUserManager</c>, a subclass of ASP.NET Identity's
...
/// </para>
/// </summary>
public interface IQuorumUserService
{
    /// <summary>
...
internal sealed class QuorumUserService : IQuorumUserService
{
    ...
    private readonly ILogger<QuorumUserService> _logger;

    public QuorumUserService(IApplicationDbContext context, ILogger<QuorumUserService> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    public async Task EnsureProvisionedAsync(string userId, string? email, CancellationToken cancellationToken)
    {
        ...
        var known = await _context.QuorumUsers.FirstOrDefaultAsync(x => x.Id == userId, cancellationToken);
        ...
        // First sight. Under the old model this ran inside QuorumUserManager.CreateAsync, because
        ...
        await _context.QuorumUsers.AddAsync(
            new QuorumUser { Id = userId, Email = email, FirstSeenAt = now, LastSeenAt = now },
            cancellationToken);
    ...
    public async Task<IReadOnlyList<string>> GetKnownUserIdsAsync(CancellationToken cancellationToken)
        => await _context.QuorumUsers.Select(x => x.Id).ToListAsync(cancellationToken);

    public async Task<string?> GetEmailAsync(string userId, CancellationToken cancellationToken)
        => await _context.QuorumUsers
            .Where(x => x.Id == userId)
            .Select(x => x.Email)
